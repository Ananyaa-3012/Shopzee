<template>
    <div class="create-category-container">
      ...
      <form @submit.prevent="submitForm" class="category-form">
        ...
        <b-form-group label="Quantity: " label-for="qty">
          <b-form-input type="number" v-model="cart.qty" id="qty" required min="1" :max="product.stock"></b-form-input>
        </b-form-group>

  
        <b-button type="submit" variant="primary">Update Cart</b-button>
      </form>
    </b-card>
    </div>
  </template>
  ...
  export default {
    ...
    methods: {
        ...
        getCart() {
        axios.get(`http://127.0.0.1:5000/cart/${this.$route.params.pdt_id}`,{
        ...
        formData.append('qty', this.cart.qty);
        formData.append('user_id', localStorage.getItem('user_id'));
  
        // Adjust the endpoint and headers based on your API
        const pdt_id = this.$route.params.pdt_id
        const apiurl = `http://127.0.0.1:5000/cart_upd/${pdt_id}`;
