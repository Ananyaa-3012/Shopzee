...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    StoreLOGIN_MUTATION(state, payload){
        state.token = payload.data.token
        state.userType = payload.data?.userType
    },
    StoreSIGNUP_MUTATION(state, payload) {
      state.token = payload.data.token
      state.user_id = payload.data.userId
    },
  ...
  actions: {
    ...
    async StoreSIGNUP_ACTION({ commit }, payload) {
      try {
        const { data } = await axios.post('http://localhost:5000/store/register', payload)
        localStorage.setItem('token', data.token)
        localStorage.setItem('user_type', data.userType)
        localStorage.setItem('user_id', data.userId)
        alert(data.message)
        router.push('/store/login')
        commit('StoreSIGNUP_MUTATION', {data})
      } catch (error) {
        if (error.response && error.response.status === 409) {
          alert('Error: '+ error.response.data.message);
          console.log(error.response.data.message);
    ...
    async StoreLOGIN_ACTION({ commit }, payload) {
        try {
          const { data } = await axios.post('http://localhost:5000/store/login', payload)
          localStorage.setItem('token', data.token)
          localStorage.setItem('user_type', data.userType)
          localStorage.setItem('user_id', data.userId)
          console.log(data)
          router.push('/store/category')
          commit('StoreLOGIN_MUTATION', { data })
        } catch (error) {
          if (error.response && error.response.status === 409) {
            alert('Error: '+ error.response.data.message);
            console.log(error.response.data.message);
